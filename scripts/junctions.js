...
function getPercentage(user, buyerName, buyerPhone, buyerEmail) {
  ...
  return (user.email == buyerEmail && (user.phone == buyerPhone || user.mobile == buyerPhone) && user.fname == buyerName)
    ? "match-100"

    // email + number + name [ masked ]
    : (isEmailMatch(buyerEmail, user.email) && isPhoneMatch(buyerPhone, user.phone || user.mobile) && user.fname == buyerName)
    ? "match-90"

    // email + phone [ masked ]
    : (isEmailMatch(buyerEmail, user.email) && isPhoneMatch(buyerPhone, user.phone || user.mobile))
    ? "match-75"
    ...
    : (isPhoneMatch(buyerPhone, user.phone || user.mobile) || user.fname == buyerName)
    ? "match-50"
